...
def ステータス補正(生ステータス: ステータス, やる気: str, コース: コース):
    やる気係数 = {
        '絶好調': 1.04,
        '好調': 1.02,
        '普通': 1.0,
...
def 初期HP(補正ステータス: ステータス, コース: コース):
    脚質係数 = {
        '逃げ': 0.95,
        '先行': 0.89,
        '差し': 1.0,
...
def 秒間体力消費_基本(状態: 状態, コース: コース):
    return 20.0 * ウマ状態係数(状態) * (状態.現在速度 - 基準速度(コース) + 12.0)**2 / 144.0 * バ場状態係数(コース)

def 秒間体力消費(状態: 状態, コース: コース, 補正ステータス: ステータス):
...
def 賢さ乱数_期待値(補正賢さ: float):
    上限 = (補正賢さ / 5500) * math.log10(補正賢さ * 0.1)
    下限 = 上限 - 0.65
    return (上限 + 下限) / 2.0

def 基準目標速度(状態: 状態, コース: コース, 補正ステータス: ステータス):
    脚質係数 = {
...
def 目標速度(状態: 状態, コース: コース, 補正ステータス: ステータス):
    # 実際にはポジションキープや移動補正などに影響されるが、未実装
    return 基準目標速度(状態, コース, 補正ステータス)

def 加速度(状態: 状態, 補正ステータス: ステータス):
    脚質係数 = {
...
def スタートダッシュ加速度(状態: 状態, 補正ステータス: ステータス):
    return 加速度(状態, 補正ステータス) + 24.0

def スタートダッシュ制限速度(コース: コース):
    return 基準速度(コース) * 0.85

def 最低速度(コース: コース, 補正ステータス: ステータス):
